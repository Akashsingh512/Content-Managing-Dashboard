...
const getAccessToken = async (serviceAccountJson) => {
  ...
  const handleStatusChange = (id, newStatus) => {
    const updatedPosts = posts.map((post) => (post.id === id ? { ...post, status: newStatus } : post));
    setPosts(updatedPosts);
    if (isConnected) {
      saveToSheets(updatedPosts);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Found in your Google Sheet URL between /d/ and /edit</p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Service Account JSON *</label>
                <textarea
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Download from Google Cloud Console → Service Accounts → Keys</p>
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button onClick={() => setShowSettings(false)} className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                  Cancel
                </button>
                <button onClick={saveSettings} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 flex items-center space-x-2">
      ...
      <div className="flex h-screen overflow-hidden">
        ...
        <div
          className={`fixed lg:static inset-y-0 left-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out z-50 w-64 bg-white border-r border-gray-200 flex flex-col`}
        >
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium transition-all ${
                activeView === 'dashboard' ? 'bg-red-50 text-red-600 border border-red-200' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium transition-all ${
                activeView === 'admin' ? 'bg-red-50 text-red-600 border border-red-200' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
            ...
            <div className="pt-4 border-t border-gray-200">
              <button onClick={() => setShowSettings(true)} className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium text-gray-600 hover:bg-gray-50 transition-all">
          ...
          <header className="bg-white border-b border-gray-200 px-4 sm:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              {activeView === 'dashboard' && (
                <button onClick={exportToCSV} className="hidden sm:flex items-center space-x-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition-colors">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-6 border-t border-gray-200">
                      <button type="button" onClick={resetForm} className="w-full sm:w-auto px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium">
                        Clear
                      </button>
                      <button
                        type="button"
                  ...
                  {statusOptions.map((status, idx) => {
                    ...
                    return (
                      <div key={status} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                        ...
                        <div className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">{posts.filter((p) => p.status === status).length}</div>
                ...
                {upcomingPosts.length > 0 && (
                  <div className="bg-gradient-to-r from-red-500 to-pink-600 rounded-xl shadow-lg p-4 sm:p-6 text-white">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="px-2 sm:px-3 py-1 bg-white bg-opacity-20 rounded-full text-xs sm:text-sm font-semibold">{upcomingPosts.length}</span>
                    </div>
                    ...
                    {filteredPosts.map((post) => (
                      <div key={post.id} className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                        <div className="p-4 sm:p-6">
                          ...
                          <select value={post.status} onChange={(e) => handleStatusChange(post.id, e.target.value)} className="w-full px-3 py-2 text-sm font-medium border rounded-lg mb-4 cursor-pointer focus:ring-2 focus:ring-red-500">
                            {statusOptions.map((status) => (
                              <option key={status} value={status}>
                                {status}
                              </option>
